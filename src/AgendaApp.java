...
public class AgendaApp {
	...
	private static void inserirContato(){
		...
		if(contatos.contains(c)){
			out.print("Este contato já está cadastrado!");
		}else {
			contatos.add(c);
	...
	private static String lerNome(){
		...
		while(!valido){
			...
			if(nome.length()==0 || nome.length()>200){
				out.print("ERRO: nome de tamanho inválido!");
			}else{ valido=true; }
		}
		return nome;
	}
	...
	private static String lerTelefone(){
		...
		while(!valido) {
			...
			if(telefone.length()==0|| telefone.length()>200){
				out.print("ERRO: telefone de tamanho inválido!");
			}else{ valido=true; }
		}
		return telefone;
	}
	...
	private static void buscarContato(){
		...
		if(resultado.size() == 0){
			out.print("Não há contato com este nome!");
		} else {
			out.println("\n Resultado da busca:");
